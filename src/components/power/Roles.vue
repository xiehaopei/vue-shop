  ...
  <div class=''>
      ...
      <el-row>
        <el-col>
          <el-button type="primary"
                     @click="showAddRoles">添加角色</el-button>
        </el-col>
      </el-row>
      ...
      <el-table :data="rolesList"
        ...
        <el-table-column type="expand">
          <template v-slot:default="SlotProp">
            <el-row v-for="(item1,i1) in SlotProp.row.children"
                    :key="item1.id"
                    :class="['bdbottom',i1===0?'bdtop':'','vertical-center']">
              <!-- 一级权限 -->
              <el-col :span="5">
                <el-tag closable
                        @close="removeRightsById(SlotProp.row,item1.id)">{{item1.authName}}</el-tag>
              ...
              <el-col :span="19">
                <el-row v-for="(item2,i2) in item1.children"
                  ...
                  <el-col :span="6">
                    <el-tag type="success"
                            closable
                            @close="removeRightsById(SlotProp.row,item2.id)">{{item2.authName}}</el-tag>
                    <i class="el-icon-caret-right"></i>
                  </el-col>
                  <el-col :span="18">
                    <el-tag v-for="item3 in item2.children"
                            ...
                            type="warning"
                            closable
                            @close="removeRightsById(SlotProp.row,item3.id)">{{item3.authName}}</el-tag>
                  </el-col>
                </el-row>
              </el-col>
            </el-row>
        ...
        <el-table-column label="操作"
          ...
          <template v-slot:default="SlotProp">
            <el-button type="primary"
                       icon="el-icon-edit"
                       size="mini"
                       @click="editRoleById(SlotProp.row.id)">编辑</el-button>
            <el-button type="danger"
                       icon="el-icon-delete"
                       size="mini"
                       @click="deleteRoleById(SlotProp.row.id)">删除</el-button>
            <el-button type="warning"
                       icon="el-icon-setting"
                       size="mini"
                       @click="showSettingRight(SlotProp.row)">分配权限</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-dialog title="分配权限"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="updateRoleRights">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async updateRoleRights () {
      ...
      const { data: res } = await this.$http.post(`roles/${this.roleId}/rights`, { rids: idArray })
